        ...
        render: function () {
            var p = this.getPosition(),
                ...
                textspan = '<span class="city-picker-span" style="' +
                    this.getWidthStyle(p.width - 205) +
                    'height:' + p.height + 'px;line-height:' + (p.height - 1) + 'px;">' +
                    (placeholder ? '<span class="placeholder" style="text-align: center">' + placeholder + '</span>' : '') +
...
//                  var address = item.address  || '';
                    if (item && type !== "province") {
                        if (type === "city") {
                            var address = item.address
                                .replace(/[市,地区,回族,蒙古,苗族,白族,傣族,景颇族,藏族,彝族,壮族,傈僳族,布依族,侗族]/g, '')
                                .replace('哈萨克', '').replace('自治州', '')
                                .replace(/自治县/, '').replace(/市/, '');
                        }
                        /*text += ($(this).hasClass('province') ? '' : '') + '<span class="select-item" data-count="' +
                            type + '" data-code="' + item.code + '">' + address + '</span>';*/
