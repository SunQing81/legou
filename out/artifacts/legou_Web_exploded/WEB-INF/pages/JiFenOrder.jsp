...
<script type="text/javascript">
    $("#goAlipayBtn").click(function () {
        ...
        var allGids = $("#goAlipayBtn").attr("jifenGid");
        var allGoodsNum = 1;
        var totalMoney = 0;
        var jifen = $("#goAlipayBtn").attr("jifen");
        ...
        if (allGids.length > 0) {
            $.ajax({
                ...
                success: function (result) {
                    // console.log(result);
                    location.href = "${APP_PATH}/JiFenOrderConfirm/" + jifen;
                }
            });
        } else {
...
<script type="text/javascript">
    //获取订单的商品
    function getJiFenGoodsByGid() {
        var gid = "${sessionScope.jifen_gid}";
        $.ajax({
            url: "${APP_PATH}/getJiFenGoodsByGid/" + gid,
            type: "GET",
            success: function (result) {
                // console.log(result);
    ...
    function builder_confirmGoods_show(result) {
        ...
        var jifenGoods = result.extendInfo.jifenGoods;
        var price = jifenGoods.gPrice.toFixed(2);

        var gDetailinfoTd = $("<td></td>").append($("<div style='float: left'></div>")
            .append($("<img style='width: 80px'>").prop("src", "${APP_PATH}/" + jifenGoods.gImage)))
            .append($("<div style='float: left;width: 350px;margin-left: 10px;font-size: 13px'></div>")
                .append(jifenGoods.gDetailinfo));
        var shoppingNum = $("<td></td>").append(1);
        var totalmoneyTd = $("<td style='font-weight: 700'></td>").append("积分：" + price * 100);
        $("<tr></tr>").append(gDetailinfoTd)
            .append(shoppingNum)
            .append(totalmoneyTd)
            .appendTo("#confirmGoodsRow table tbody");
        $("#goAlipayBtn").attr("jifenGid", jifenGoods.gId).attr("jifen", +price * 100);
        $("#confirmOrderRow").find("tr:first").children("td").text("共 " + 1 + " 件商品");
        $("#confirmOrderRow").find("tr:first").next().children("td").find("span").text("积分：" + price * 100);
    }
</script>
<%--地址展示以及功能--%>
<script type="text/javascript">
